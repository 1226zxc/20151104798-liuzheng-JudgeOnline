package com.lz.web.controller.Interceptor;
...
public class AdminPermissionInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		AdminPermission permission = method.getMethodAnnotation(AdminPermission.class);
		// 如果为空在表示该方法不需要进行权限验证
		if (permission == null) {
			return true;
		}
		...
		if (manager.getRole() == null ||
				!checkManagerPermission(manager.getRole().getPermissions(), permission.value())) {
	...
	private boolean checkManagerPermission(String managerPermissions, Permissions[] permissions) {
